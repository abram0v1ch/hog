...
def tail_points(opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    result = 2 * abs(opponent_score // 10 - opponent_score % 10) + 1
    return tail_points(result) if result > 999 else result % 10 if result > 20 else result
    # END PROBLEM 2
...
def next_perfect_square(number: int):
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    return strategy(1, 20) == strategy(60, 0) == strategy(0, 60) == strategy(60, 60) == strategy(99, 99) == strategy(0, 0)
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    minimum_roll = 10
    while count <= 10:
        value = average(count, dice)
        if value > maximum:
            maximum = value
            minimum_roll = count
        elif value == maximum and count < minimum_roll:
            minimum_roll = count
        #  end if
        count += 1
    #  end while
...
def final_strategy(score, opponent_score, goal=GOAL):
    """This strategy returns 0 dice when your score would be >= goal
    or if that would make <7 points to win.
    It returns 1 if the player lacks <7 point to win.
    It returns 2 if the player lacks <13 points to win.
    Otherwise it returns 6.

    Strategy is good to use for endgames.
    """
    # BEGIN PROBLEM 12
    difference = goal - score
    tail_value = simple_update(0, score, opponent_score)
    if goal < tail_value or goal - tail_value < 7:
        return 0
    elif difference < 7:
        return 1
    elif difference < 13:
